...
var Cafe = {
  ...
  formatPrice: function(price) {
    return Cafe.formatNumber(price, 2, '.', ',') + ' UZS';
  },
    ...
    if (Cafe.modeOrder) {
      ...
      Cafe.apiRequest(
        ...
        function(result)
        {
          $.ajax({
            type: 'POST',
            data: Cafe.getOrderData(),
            dataType: 'json',
            // url: 'https://bf95-213-230-116-199.ngrok.io/good?str=XsSalomSalom',
            xhrFields: {
              withCredentials: true
            },
            success: function(result) {
              onCallback && onCallback(result);
              // Cafe.showStatus(this.data + '!!Success');
            },
            error: function(xhr) {
              Cafe.showStatus(this.data + '!!Error berdi');
            }
          });
      ...
      //   }
      //   if (result.error) {
      //     Cafe.showStatus(result.error + '|' + params.user_hash + '|' + Cafe.userHash);
      //   }
      // });
    } else {
